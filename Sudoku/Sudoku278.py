...
def csp(cons):
    ...
    for val in cons[var]:
        new_constraints = sudoku_logic_1(sudoku_logic_2(modify_possible(cons, var, val)))
        if impossible_state(new_constraints):
            continue
        call_count += 1
        result = csp(new_constraints)
...
#
#
# def csp_ext(state, cons): # Longer Extension
#     global call_count
#     if goal_test(state):
#         return state
#     var = get_next_unassigned_var(cons)
...
for line in file:
    ...
    print(str(N) + " " + board + " " + str(end - start) + "s " + str(call_count) + "calls " + str(correct()))
    call_count = 1
    count += 1
print("Total " + str(total) + "s " + str(total_calls) + "calls")
